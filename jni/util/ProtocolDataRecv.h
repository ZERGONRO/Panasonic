...
//	BYTE TimingSwitchModeSelect;		//定时开关模式选择 		净化器-空调-全热交换器-调湿器-新风换气-除菌除味-浴霸
	...
	BYTE ERVPreventFreezing;			//erv防止冻结标志	0-1
	BYTE AirCondiOutdoorDefrost;		//空调外机除霜标志	0-1
	BYTE PreventMode;					//保护模式标志		0-1
	BYTE AdjustHumdPreCondensation;		//调湿防结霜标志		0-1
	BYTE AdjustHumdDry;					//调湿干燥标志		0-1
...
typedef struct AdjustHumdRecvData_t {
	...
	BYTE FunctionSelectSetting;		//功能选择		0自动除湿-1连续除湿-2保湿-3送风
}AdjustHumdRecvData;
